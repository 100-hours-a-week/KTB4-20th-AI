...
async def generate_structured(
    ...
    # 실패(예외 또는 parsed=None) 시 1회 재시도, 그래도 실패하면 마지막 에러를 그대로 올림

    contents = user if isinstance(user, list) else [user]
    ...
    for _ in range(MAX_ATTEMPTS):
        ...
        last_error = ValueError("Gemini 응답이 스키마와 맞지 않음 (parsed=None)")
    raise last_error
