...
| 포토미션 검증 모듈 | POST /photomissions/{mission_id}/verify | ① 위치 사전 확인 → ② 사진 판정(VLM) + 등급 변환                             | v1      |
...
담당 기능: 장소명과 반영된 취향을 받아, 미션 문구(`description`)와 완료 방식(`scope`)을 한 번의 호출로 함께 정한다. 장소가 여러 개여도 장소 전부의 미션을 한 번의 호출로 받는다.
...
담당 기능: 요청으로 받은 사진 좌표(`photo_coordinates`)가 목표 장소와 **명백히 불일치하는 경우에만** 사진을 내려받지 않고 VLM 호출 없이 조기 반려한다. 좌표가 가깝거나 좌표가 없으면 ②로 넘긴다.

좌표를 성공 판정에 쓰지 않는 이유: 위치가 맞아도 사진이 미션을 수행했는지는 알 수 없기 때문이다(첨성대 앞에서 찍었지만 미션과 무관한 사진일 수 있음). 좌표는 명백한 오답을 걸러 다운로드와 VLM 호출을 아끼는 용도이고, 성공 판정은 ②의 VLM이 담당한다.

입력 스키마
```python
class LocationCheckInput(BaseModel):
    photo_coordinates: Coordinates | None  # 요청에서 받은 값 그대로, 없으면 None
    place_coordinates: Coordinates         # 요청에서 받은 값 그대로
```

입력 예시
```json
{ "photo_coordinates": { "latitude": 35.8350, "longitude": 129.2195 }, "place_coordinates": { "latitude": 35.8347, "longitude": 129.2192 } }
```

출력 스키마
```python
class LocationCheckOutput(BaseModel):
    needs_vlm: bool  # 명백히 불일치할 때만 False
```
...
| 사진 좌표 | 목표 좌표와의 거리 | `needs_vlm` | 처리 |
|---|---|---|---|
| 없음 | - | `true` | ②로 넘김 |
| 있음 | 가까움 | `true` | ②로 넘김 — 위치는 맞아도 사진 내용 확인 필요 |
| 있음 | 명백히 불일치 | `false` | 조기 반려(`fail`), 사진 다운로드·VLM 호출 생략 |

> "명백히 불일치"의 기준 거리는 2단계 평가셋으로 확정한다(현재 임시값 5km). 위치 정보가 없는 사진도 있어(위치 권한 거부, 메신저 전달, 스크린샷), 조기 반려는 어디까지나 보조 수단이다.

다음 단계와의 연결: `needs_vlm`이 `true`면 ②(VLM)가 호출된다. `false`는 사진 좌표가 명백히 불일치한 경우뿐이며, 이때만 VLM 없이 `fail`로 확정된다.
...
①단계(위치 사전 확인)에서 사진 좌표가 명백히 불일치해 조기 반려되는 경우, ②단계(이 절)를 거치지 않고 같은 출력 형태로 result="fail", reason="location_mismatch"가 바로 반환된다. 그 외의 모든 경우 reason은 null이다.
