...
입력 예시
```json
{ "id": "ChIJq6q6q6q6q6qRZ0000000000", "selected_for": ["u_jiyoung"] }
```
...
출력 예시
```json
{ "id": "ChIJq6q6q6q6q6qRZ0000000000", "scope": "individual" }
```
...
- `scope`: `group`이면 여럿이 함께 수행하는 형태로, `individual`이면 혼자 수행하는 형태로 문구를 만든다.
...
출력 예시
```json
{ "mission_id": "ms_002", "place_id": "ChIJq6q6q6q6q6qRZ0000000000", "description": "황리단길 한옥 골목 사진 찍기", "scope": "individual", "primary_category": "HISTORY_CULTURE" }
```

- primary_category는 이 단계(LLM)의 출력이 아니다. matched_preferences의 첫 번째 값을 그대로 옮기는 규칙이며, 응답을 최종 조립하는 시점에 결정론적으로 채워진다. (①단계의 scope 판정과 같은 성격이다 — 규칙으로 답이 정해지는 판단이므로 모델에 맡기지 않는다.)
...
class VerifyOutput(BaseModel):
    ...
    landmark_confidence: float | None     # 임계값 미만이면 null
