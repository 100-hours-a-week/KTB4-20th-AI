...
```mermaid
flowchart LR
    A["요청 수신<br/>places[]"] --> S1["1. 미션 문구·유형 생성 (LLM)<br/>입력: 장소명 + matched_preferences<br/>출력: description + scope<br/>(장소 전부를 1회 호출)"]
...
| # | 단계 | 책임 | 입력 → 출력 | 방식 | 병목 |
|---|---|---|---|---|---|
| 1 | 미션 문구·유형 생성 | **사진으로 판정 가능한** 행동을 한 문장으로 작성하고, 대표 사진 한 장으로 전원 완료되는지(`GROUP`) 각자 제출해야 하는지(`PERSONAL`)를 함께 정함 | 장소·취향 → `description`·`scope` | **LLM** | decode - 한 번의 호출로 장소 전부의 미션을 받으므로 출력 토큰이 장소 수에 비례 |
...
```mermaid
flowchart TB
    A["요청 수신<br/>image_url, place_coordinates, photo_coordinates,<br/>place_name, mission_description"]
    A --> S1{"1. 입력 검증 (규칙)<br/>허용 S3 호스트인가?"}
    S1 -- 아니오 --> X["422 거부<br/>(SSRF 방지)"]
    S1 -- 예 --> S2{"2. 위치 사전 판정 (규칙)<br/>photo_coordinates가<br/>목표 좌표와 명백히 불일치?"}
    S2 -- "예" --> S2a["조기 반려<br/>match_score = 0<br/>(3~5번 생략: 다운로드·모델 호출 없음, landmark·hint는 None)"]
    S2 -- "아니오 / 좌표 없음" --> S3["3. 이미지 획득 (I/O)<br/>presigned URL로 다운로드"]
    S3 --> S4["4. 이미지 전처리 (규칙)<br/>리사이즈·포맷 정규화"]
    S4 --> S5["5. 사진 판정 (VLM)<br/>입력: 이미지 + place_name + mission_description<br/>출력: VlmResult"]
    S5 --> S6{"6. 등급 변환 (규칙)<br/>match_score 구간"}
    S6 -- "높음" --> S6a["result = success"]
    S6 -- "애매" --> S6b["result = retry"]
    S6 -- "낮음" --> S6c["result = fail"]
    S6a --> S7["7. 응답 조립 (규칙)<br/>임계값·등급으로 노출 여부 결정<br/>스키마 구성 (문구는 안 만듦)"]
    S6b --> S7
    S6c --> S7
    S2a --> S6
```
...
| 2 | 위치 사전 판정 | 사진 좌표가 목표 좌표와 명백히 먼지 판단. 좌표가 없으면 판단하지 않고 넘김 | (`photo_coordinates`, 목표 좌표) → 조기 반려 여부 | 규칙 | 없음(거리 계산) |
| 3 | 이미지 획득 | presigned URL로 원본 바이트 확보 | `image_url` → `bytes` | I/O | S3 다운로드 - 서버 간 통신이라 빠를 것으로 예상되나 원본 해상도가 크면 실측 필요 |
| 4 | 이미지 전처리 | 리사이즈·포맷 정규화로 입력 크기 축소 | `bytes` → `bytes` | 규칙 | 인코딩(prefill) - 2단계에서 식별한 1순위 병목 |
| 5 | 사진 판정 | 인식 라벨 → 랜드마크 인식 → 미션 점수 → 재촬영 힌트를 한 번의 호출로 산출 | (이미지, 장소명, 미션 내용) → `VlmResult` | **VLM** | decode - 출력이 짧아 `generate`보다 비중은 작을 것으로 예상 |
| 6 | 등급 변환 | 점수를 success/retry/fail로 매핑 | `match_score` → `result` | 규칙 | 없음(임계값 비교) |
| 7 | 응답 조립 | 점수로 6번을 호출해 등급을 정하고, 오케스트레이터가 건넨 원값에 임계값·등급을 적용해 `landmark_confidence`·`retry_hint`의 노출 여부를 정하고 스키마를 구성 (안내 문구는 만들지 않음 — 아래 참고) | 위 결과 → `VerifyResponse` | 규칙 | 없음(임계값·조건 비교) |

**모델이 들어가는 자리는 두 곳뿐이다.** `generate`의 1번과 `verify`의 5번. 나머지 일곱 단계는 전부 규칙으로 답이 정해진다.
...
| Pillow (리사이즈) | 리사이즈·포맷 변환·방향 보정을 한 라이브러리로 처리할 수 있고, 4번 단계가 그것뿐이라 별도 이미지 서비스를 둘 이유가 없다 | 인코딩(prefill) 병목을 앞단에서 축소. 클라이언트가 원본을 보낸 경우의 2차 방어선 역할 |
| Haversine 공식 (직접 구현) | 2번 단계는 두 좌표 간 직선 거리만 필요하고, "명백한 불일치"(수십 km 차이)만 판별하면 되므로 지도 API 호출이 불필요 | 외부 API 호출 없이 오답 케이스를 걸러 3번(다운로드)과 5번(모델 호출)을 통째로 생략 |

**LangChain / LangGraph를 쓰지 않는 이유**

두 프레임워크가 푸는 문제(체인 연결, 상태 기반 분기·순환)가 이 파이프라인에 없다.

- **체인**: 모델 호출은 엔드포인트마다 한 번이고, 한 호출의 출력을 다른 호출의 입력으로 넘기는 일이 없어 엮을 것이 없다.
...
async def generate_missions(request: PhotoMissionGenerateRequest) -> PhotoMissionGenerateResponse:
    batch = await write_descriptions(request.places)                        # 1
    # 모델이 돌려준 순서와 상관없이, 번호로 요청의 장소와 짝짓는다
    by_number = {m.number: m for m in batch.missions}
    missions = [
        build_mission(place, by_number[number])                             # 2
...
async def write_descriptions(places: list[MissionPlace]) -> MissionBatch:
    ...
    return await generate_structured(
        ...
        user=user_prompt,
        response_schema=MissionBatch,   # Pydantic 모델을 그대로 넘긴다
...
Gemini 호출 에러는 HTTP 상태 코드로 바꿔 돌려준다(429 → 429 + `Retry-After`, 그 외 → 502, 타임아웃 → 504). `verify`의 5번도 같다.
...
async def verify_photo(request: VerifyRequest) -> VerifyResponse:
    ...
    if request.photo_coordinates is not None and is_clear_mismatch(             # 2
        ...
        # 3~5번을 건너뛴다. 다운로드·모델 호출 없이 끝나는 유일한 경로. 점수 0은 6번에서 fail이 된다.
        return build_response(match_score=0.0, reason="location_mismatch")

    image_bytes = await fetch_image(request.image_url)                          # 3
    processed = normalize_image(image_bytes)                                    # 4
    vlm = await score_photo(                                                    # 5
        processed, request.place_name, request.mission_description
    )

    # 오케스트레이터는 판단하지 않는다. VLM 원값을 그대로 넘기고, 등급(6번)은 7번 안에서 정한다.
    return build_response(                                                      # 6·7
        vlm.match_score, vlm.detected_labels,
...
def is_clear_mismatch(photo_coordinates: Coordinates, target: Coordinates) -> bool:
    """2. 위치 사전 판정 — 사진 좌표가 목표 장소에서 명백히 먼지만 본다."""
    return haversine_km(photo_coordinates, target) > CLEAR_MISMATCH_KM


def to_grade(match_score: float) -> Literal["success", "retry", "fail"]:
    """6. 등급 변환 — 임계값 규칙만. 모델을 호출하지 않는다."""
...
def build_response(
    match_score, detected_labels=None,
    raw_landmark_confidence=None, raw_retry_hint=None, reason=None,
) -> VerifyResponse:
    """7. 응답 조립 — landmark_confidence·retry_hint의 노출 여부를 임계값·등급으로 정하고
    스키마를 구성한다. 문구는 붙이지 않는다."""
    # 등급은 점수로만 정해지므로 따로 받지 않고 6번을 호출한다.
    # 등급을 인자로 받으면 build_response("success", 10)처럼 점수와 어긋난 응답을 만들 수 있다.
...
class VlmResult(BaseModel):
    ...
    retry_hint: str                       # 조언 — 항상 생성. 노출 여부는 7번이 정한다


async def score_photo(image: bytes, place_name: str, mission_description: str) -> VlmResult:
    """5. 사진 판정 (VLM) — 점수·라벨·랜드마크 인식 여부·힌트를 한 번에 낸다.
    등급(success/retry/fail)은 6번이 정한다."""
...
`SUCCESS_THRESHOLD` / `RETRY_THRESHOLD` / `LANDMARK_THRESHOLD` / `CLEAR_MISMATCH_KM`은 2단계 평가셋(경주 4곳 400건, 사람이 매긴 정답 200장)으로 `gemini-3.1-pro-preview` 기준 90 / 40 / 60 / 5km로 확정했다. 점수 분포는 모델마다 달라서(같은 평가셋에서 `gemini-3.1-flash-lite`는 성공 기준 80이 맞았다) 모델을 바꾸면 다시 측정한다.

### 멀티스텝 도입 근거

**원칙 1: 한 단계는 한 가지만 판단한다.**

6번(등급 변환)과 7번(응답 조립)은 한 덩어리로 묶을 수도 있었지만 나눴다. 6번은 점수를 등급으로 바꾸는 판정이고, 7번은 VLM 원값에 임계값·등급을 적용해 랜드마크·retry_hint의 노출 여부를 정하고 응답 스키마를 구성하는 조립이다. 다만 7번은 등급을 인자로 받지 않고 점수로 6번을 직접 호출한다. 등급은 점수에서만 나오는 값이라, 둘을 따로 받으면 서로 맞지 않는 조합이 응답에 들어갈 수 있기 때문이다. 안내 문구는 7번도 만들지 않는다 — AI는 미션의 카테고리(유적/음식/활동 등)를 모른다. `verify` 요청에 `matched_preferences`가 없기 때문이다. 대신 `result`·`reason`과 `generate` 응답의 `Mission.primary_category`를 프론트엔드가 조합해 문구를 만든다. AI가 만드는 값은 등급·사유뿐이고, 문장으로 옮기는 일은 그 값을 소비하는 쪽이 한다.

**원칙 2: 규칙으로 답이 정해지는 판단은 모델에 맡기지 않는다.**
| 판단 | 답을 정하는 방법 | 모델에 맡기면 |
|---|---|---|
| 위치 명백 불일치 | 좌표 간 거리 계산 | 답이 확정적인 케이스에 비싼 VLM을 호출하게 됨 |
...
- **비용·latency 절감**: 2번에서 명백한 오답을 걸러내면 3번(사진 다운로드)과 5번(유일한 모델 호출)이 통째로 사라진다. 이미지 인코딩 때문에 VLM은 텍스트 호출보다 비싸므로 앞단에서 거르는 이득이 크다.
...
- **임계값 조정이 프롬프트와 분리됨**: 2단계 평가셋으로 임계값을 확정할 때 6번 함수만 바꾸면 되고 5번 프롬프트는 건드리지 않는다. VLM은 점수만 내고 등급은 코드가 정하기 때문이다.
- **교체 범위가 한 단계로 한정됨**: v1 프론티어 API에서 v3 자체 서빙(Qwen2.5-VL)으로 바꿔도 교체 대상은 5번뿐이고, 저장소를 S3에서 다른 것으로 바꿔도 3번뿐이다 — 3단계 "모델 교체 시 영향 범위" 사례와 같은 이유다.

**서비스 요구와의 관련성**

`verify`는 사용자가 여행 중 화면을 손에 쥔 채 결과를 기다리는 엔드포인트다. 대기가 길어지면 결과를 확인하지 않고 다음 장소로 이동해버리는 이탈로 이어지므로, 2단계에서 비동기 처리를 기각하고 동기 응답을 유지하기로 했다. 2번 조기 반려는 그 제약 안에서 일부 요청을 모델 호출 없이 더 빨리 끝내는 장치다.

`retry` 등급을 따로 둔 것도 서비스 요구에서 나왔다. 성공/실패 이분법이면 애매한 점수의 사진이 사용자에게 설명 없이 실패로만 보인다. 6번에서 세 등급으로 나누고 7번에서 `retry`일 때만 `retry_hint`를 내려주기 때문에, 프론트엔드가 "다시 찍어보세요"라는 안내와 구체적인 재촬영 힌트를 보여줄 수 있다.
