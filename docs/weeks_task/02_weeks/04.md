...
```mermaid
flowchart LR
    A["요청 수신<br/>places[]"] --> S1{"1. 미션 유형 판정 (규칙)<br/>selected_for 인원 수"}
    S1 -- "정확히 1명" --> S1a["scope = individual"]
    S1 -- "0명 또는 2명 이상" --> S1b["scope = group"]
    S1a --> S2["2. 미션 문구 생성 (LLM)<br/>입력: 장소명 + scope + matched_preferences<br/>출력: description"]
    S1b --> S2
    S2 --> S3["3. 응답 조립 (규칙)<br/>mission_id 부여<br/>primary_category 결정"]
    S3 --> B["missions[]"]
```
...
| #   | 단계           | 책임                                                                                    | 입력 → 출력                       | 방식    | 병목                                                              |
| --- | -------------- | --------------------------------------------------------------------------------------- | --------------------------------- | ------- | ----------------------------------------------------------------- |
| 1   | 미션 유형 판정 | 다 같이 하는 미션인지 혼자 하는 미션인지 결정                                           | `selected_for` → `scope`          | 규칙    | 없음(배열 길이 비교)                                              |
| 2   | 미션 문구 생성 | **사진으로 판정 가능한** 행동을 한 문장으로 작성                                        | 장소·`scope`·취향 → `description` | **LLM** | decode - 장소 수만큼 문구를 생성하므로 출력 토큰이 장소 수에 비례 |
| 3   | 응답 조립      | `mission_id` 부여, `matched_preferences[0]`로 `primary_category` 결정, 응답 스키마 구성 | 위 결과 → `Mission`               | 규칙    | 없음                                                              |
...
**모델이 들어가는 자리는 두 곳뿐이다.** `generate`의 2번과 `verify`의 6번. 나머지 아홉 단계는 전부 규칙으로 답이 정해진다.
...
| Qwen2.5-VL-7B-Instruct (GGUF Q4_K_M)             | 2단계에서 `verify`를 로컬 전환 1순위로 지정(실시간 제약·GPU 부하가 모두 큼). 텍스트·비전을 한 모델로 처리해 서빙 구성이 단순함                                                                                          | 호출당 과금에서 GPU 고정비로 전환. 단, 양자화 시 정확도 유지가 관건이라 평가셋 점수로 전환 시점을 정함                                                                                |
...
| Haversine 공식 (직접 구현) 또는 `geopy.distance` | 4번 단계는 두 좌표 간 직선 거리만 필요하고, "명백한 불일치"(수십 km 차이)만 판별하면 되므로 지도 API 호출이 불필요                                                                                                      | 외부 API 호출 없이 오답 케이스를 걸러 6번(모델 호출)을 통째로 생략                                                                                                                    |

**LangChain / LangGraph를 쓰지 않는 이유**

두 프레임워크가 푸는 문제(체인 연결, 상태 기반 분기·순환)가 이 파이프라인에 없다.

- **체인**: verify의 모델 호출은 VLM 판정(6번)과 랜드마크 정보 생성(8번) 둘인데, 8번은 6번의 출력을 쓰지 않는다. "사진이 미션대로 찍혔는가"와 "이 장소가 어떤 곳인가"는 서로 독립된 질문이라 엮을 것이 없다.
...
async def generate_missions(request: PhotoMissionGenerateRequest) -> PhotoMissionGenerateResponse:
    ...
    for place in request.places:
        scope = judge_scope(place.selected_for)                     # 1
        description = await write_description(place, scope)         # 2
        missions.append(build_mission(place, scope, description))   # 3
...
def judge_scope(selected_for: list[str]) -> Literal["group", "individual"]:
    """1. 미션 유형 판정 — 인원 수만 본다. 모델을 호출하지 않는다."""
    return "individual" if len(selected_for) == 1 else "group"


async def write_description(place: MissionPlace, scope: str) -> str:
    """2. 미션 문구 생성 — 행동만 담은 문구를 만든다. mission_id는 여기서 다루지 않는다.
...
def build_mission(place: MissionPlace, scope: str, description: str) -> Mission:
    """3. 응답 조립 — mission_id를 부여하고 스키마를 구성한다."""
    return Mission(
...
여행자가 그 장소에서 찍을 사진 미션 문구를 하나 만든다.
...
6. matched_preferences로 담을 대상의 종류를 정한다. 취향 이름 자체는 문구에 쓰지 않는다.
  ...
  둘을 한 문구에 억지로 담지 않는다.
7. scope가 group이면 여러 사람이 함께 찍는 미션으로,
   individual이면 각자 찍는 미션으로 쓴다.
   - group 좋음: 다 같이 능선을 배경으로 사진 찍기
   - group 나쁨: 4명이 모두 나오게 찍기   <- 인원수를 넣지 않는다(규칙 4)
...
def assert_allowed_source(image_url: str) -> None:
    ...
    if urlparse(image_url).hostname not in ALLOWED_IMAGE_HOSTS:
        raise HTTPException(422, "image_url must be from the allowed S3 bucket host")
...
class VlmResult(BaseModel):
    ...
    landmark_confidence: float            # 인식 — 그것이 목표 장소인가 (0~1)
    match_score: float                    # 판단 — 미션대로 찍혔는가 (0~1)
...
async def score_photo(image: bytes, place_name: str, mission_description: str) -> VlmResult:
    ...
    return await gemini.generate(
        ...
        user=[user_prompt, image],            # 텍스트 + 이미지
...
| --------------------------------- | --------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------- |
| `scope` (group/individual)        | `selected_for` 배열 길이          | 같은 입력에 다른 답이 나올 수 있음. 규칙이 한 줄인데 호출 비용과 변동성만 추가됨                                                             |
...
- **재현성과 테스트 용이성**: 규칙 단계는 모델 없이 단위 테스트가 가능하다. `judge_scope`, `is_clear_mismatch`, `to_grade`, `build_response`는 입력만 주면 항상 같은 답이 나오므로, 모델 응답을 모킹하지 않고도 검증할 수 있다.
...
`retry` 등급을 따로 둔 것도 서비스 요구에서 나왔다. 성공/실패 이분법이면 애매한 점수의 사진이 사용자에게 설명 없이 실패로만 보인다. 7번에서 세 등급으로 나누고 8번에서 등급별 안내 문구를 붙이기 때문에 "다시 찍어보세요"라는 안내가 가능해진다.
